...
module RailsEventStore
  ...
  class HandlerWithDefaults_ < ActiveJob::Base
  ...
  class HandlerWithAnotherEventStore_ < ActiveJob::Base
    cattr_accessor :event
  ...
  RSpec.describe AsyncIdOnlyHandler do
    let(:event_store) { RailsEventStore::Client.new }
    let(:another_event_store) { RailsEventStore::Client.new }
    ...
    specify "with defaults" do
      HandlerWithDefaults_.prepend RailsEventStore::AsyncIdOnlyHandler
      handler = HandlerWithDefaults_.new

      HandlerWithDefaults_.event = nil
      event_store.subscribe_to_all_events(HandlerWithDefaults_)
      event_store.publish(ev = RailsEventStore::Event.new)
      wait_until{ HandlerWithDefaults_.event }
      expect(HandlerWithDefaults_.event).to eq(ev)
    end

    specify "with specified event store" do
      HandlerWithAnotherEventStore_.prepend RailsEventStore::AsyncIdOnlyHandler.with(event_store: another_event_store)
      handler = HandlerWithAnotherEventStore_.new

      HandlerWithAnotherEventStore_.event = nil
      event_store.subscribe_to_all_events(HandlerWithAnotherEventStore_)
      event_store.publish(ev = RailsEventStore::Event.new)
      wait_until{ HandlerWithAnotherEventStore_.event }
      expect(HandlerWithAnotherEventStore_.event).to eq(ev)
    end

    # specify 'ActiveJob with AsyncHandler prepended' do
    #   HandlerWithHelper.prepend RailsEventStore::AsyncIdOnlyHandler
    ...
    #   HandlerA = Class.new(MetadataHandler)
    #   HandlerA.prepend RailsEventStore::CorrelatedHandler
    #   HandlerA.prepend RailsEventStore::AsyncIdOnlyHandler
    ...
    #   HandlerB = Class.new(MetadataHandler)
    #   HandlerB.prepend RailsEventStore::CorrelatedHandler
    #   HandlerB.prepend RailsEventStore::AsyncIdOnlyHandler
    ...
    #   ev = RailsEventStore::Event.new
    #   Sidekiq::Testing.fake! do
    #     SidekiqHandlerWithHelper.prepend RailsEventStore::AsyncIdOnlyHandler.with
